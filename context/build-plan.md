...
### 03 PostHog Initialization — built 2026-07-30
...
> **Built differently, deliberately — do not "fix" this back.** Two bullets above are superseded by Next.js 16 and the installed PostHog skill:
>
> - **Init lives in `instrumentation-client.ts`, not the root layout.** Next 16's client bootstrap hook runs before any component mounts, so no provider component is needed.
> - **`lib/posthog-client.ts` does not call `init()`.** It is the typed capture surface — an `AnalyticsEvent` union plus `captureEvent()`, `captureError()`, `identifyUser()`, `resetUser()`.
>
> `identify()` also runs on every signed-in render rather than once "after login", via `PostHogIdentity` in the root layout — same outcome, and it survives a page refresh.
>
> Five auth lifecycle events were added on top of this feature; the four product events in `code-standards.md` belong to Features 06/10/13 and could not be instrumented yet. See `progress-tracker.md § Feature 03`.
