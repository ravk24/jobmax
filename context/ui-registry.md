...
**External links are guaranteed http(s) by the schema, not by the component.** `JobHeader` and `JobActions` render `external_apply_url` straight into `href` (always `target="_blank" rel="noopener noreferrer"`) because `jobDetailSchema` scheme-guards it — a non-http(s) value degrades to `null` and hits the existing disabled-button branch. Any future externally-sourced URL that becomes a link — a dossier's company website, a manual import — gets the same guard at its schema, never ad-hoc validation in the component.
