...
Source: `context/design/dashboard.png`. The stat cards and the activity feed read real data via `lib/dashboard-query.ts` (Features 15/16); the three charts render mock data from `lib/dashboard-mock.ts` until Feature 17; the incomplete-profile banner is real.
...
| Card empty state | one muted line in the card body: `p-6 text-sm leading-5 text-text-muted` (RecentActivity, Feature 16) — Feature 17's chart empty states should match this, not invent a medallion |
