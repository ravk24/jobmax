...
Source: `context/design/dashboard.png`. **Everything is real data**: stat cards and activity via `lib/dashboard-query.ts` (Features 15/16), the three charts via `lib/posthog-query.ts` (Feature 17, PostHog Query API). `lib/dashboard-mock.ts` is deleted.
...
| Card empty state | one muted line: `text-sm leading-5 text-text-muted` — in the card body at `p-6` (RecentActivity, Feature 16) or centred in the chart's held `h-[280px]` slot (`flex items-center justify-center`, all three charts, Feature 17) so the grid footprint never shifts. No medallion |
...
**Charts degrade the same way the stat cards and the feed do (Feature 17).** Each chart takes `emptyMessage?: string` and renders the card-empty-state line in its held 280px slot when `data` is empty; the page distinguishes "Chart couldn't be loaded." (read error) from per-chart no-data copy (all-zero counts — the lib zero-fills, so ok-but-all-zero *is* the empty condition). Axis labels, hover tooltips and auto-scaled domains are untouchable — they are the contrast-WARN relief. Chart data arrives as raw dates/counts from `lib/posthog-query.ts`; date and weekday labels are built in the page (`formatChartDay`, UTC-pinned), the buildStats split.
**The activity feed formats in the page, not in the component or the lib (Feature 16).** `RecentActivity` stays presentational; `lib/dashboard-query.ts` returns raw `ActivityItem`s; `activityMessage()`/`formatTimeAgo()` in `app/dashboard/page.tsx` compose "Found X jobs for Y" (singular "1 job") and the mock's time vocabulary (Just now / mins / hours / Yesterday / days — rolling, not calendar). The card never hides: `emptyMessage` renders one muted `p-6` line — "No activity yet — search for jobs to get started." when the read succeeded empty, "Activity couldn't be loaded." when it failed — and the `<ol>` renders only when entries exist. A completed search that found 0 jobs is a real entry ("Found 0 jobs for …"), not a hidden one; failed runs are excluded at the query.
